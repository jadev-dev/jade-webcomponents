...
export class MoreDetails extends HTMLElement {
  ...
  disconnectedCallback() {
    this._detailToggle.removeEventListener('click', () => this._onClick());
    this._detailsSlot.removeEventListener('slotchange', () => {
      this._slotChange();
    });
  }
  ...
  _allDetails() {
    return Array.from(
      this.querySelectorAll('*[slot=details]')
  ...
  _updateHiddenState() {
    ...
    this._detailToggle.textContent = this.toggled
      ? 'fewer details...'
      : 'more details...';
  }
}
